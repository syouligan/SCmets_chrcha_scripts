...
phate.out <- phate(Matrix::t(GetAssayData(filtered_exp.integrated, assay = "integrated", slot = "scale.data")), ndim = 2) # Runs PHATE diffusion map
...
filtered_exp.integrated <- FindNeighbors(filtered_exp.integrated, reduction = "phate", dims = 1:2)
...
for(i in c("pca", "umap")) {
  ...
  p3 <- FeaturePlot(filtered_exp.integrated, reduction = i, features = c("digest_stress1"), order = TRUE)
  p4 <- FeaturePlot(filtered_exp.integrated, reduction = i, features = c("dying1"), order = TRUE)
  p5 <- FeaturePlot(filtered_exp.integrated, reduction = i, features = c("Mito_percent"), order = TRUE)
  p6 <- FeaturePlot(filtered_exp.integrated, reduction = i, features = c("Lib_size"), order = TRUE)
