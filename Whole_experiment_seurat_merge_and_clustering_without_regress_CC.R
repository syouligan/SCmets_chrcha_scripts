...
for (i in 1:length(filtered_exp.list)) {
  filtered_exp.list[[i]] <- SCTransform(filtered_exp.list[[i]], verbose = TRUE, vars.to.regress = c("Lib_size", "S.Score", "G2M.Score"))
}
...
for(i in c("pca", "umap")) {
  ...
  ggsave(paste0("Seurat_clusters_", i, "_no_regress_stress.png", plot = gridit), device = "png")
}
...
for(i in c("pca", "umap")) {
  ...
  ggsave(paste0("PHATE_clusters_", i, "_no_regress_stress.png", plot = gridit), device = "png")
}
